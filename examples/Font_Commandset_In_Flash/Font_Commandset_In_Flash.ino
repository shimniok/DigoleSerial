...
void setup()
{
  ...
  mydisp.setFont(0);
  flashErase(saddr,8588);  //erase 8588 bytes in flash to store font data, this function will use screen memory to buffer the 4096 erase block
  delay(400);
  flashWrite(saddr, 8588, userfont3);  //write array d to flash memory start at 0, length=8588
  setFlashFont(saddr);
  mydisp.drawStr(0,0,"abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUXYZ");
  delay(3000);
  mydisp.clearScreen();
  //Test command set in flash memory
  flashErase(4096,4096);  //if you erase the whole blocks, the module will not use the screen buffer to save data
  flashWrite(4096, sizeof(commandset),commandset);
  runCommandSet(4096);  //run command set of: commandset
  delay(4000);
  flashErase(0x1ff000,sizeof(DigoleStartScreen)+3);  //another command set to show Digole welcome screen
  flashWrite(0x1ff000, sizeof(DigoleStartScreen), DigoleStartScreen);
  runCommandSet(0x1ff000);  //run command set of: DigoleStartScreen
  flashReadStart(8180,20);  //read flash memory, you can see what in flash memory begining from address of 1000 (commandset)
  for(int i=0;i<20;i++)
  {
    b=mydisp.read();
    Serial.print(b);
